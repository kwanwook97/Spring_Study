...
/*
	...
	<c:if test="overlaypass == true">
	<th colspan =2><button>회원가입</button></th>
	</c:if>
	<c:if test="overlaypass == false">
	<th colspan =2><button type="button" onclick="check()">회원가입</button></th>
	</c:if>
	
	function listPrint(){
		var content = '';
			if(overlaypass == false){
			
				content += '<th colspan =2><button type="button" onclick="check()">회원가입</button></th>';	
			}
			else if (overlaypass == true){
				content += '<th colspan =2><button>회원가입</button></th>';
			}

		$('#boardTable').html(content);
	}
